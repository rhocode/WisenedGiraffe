...
export default class App2 extends Component {
  ...
  generateMeme(d3, svg) {
    ...
    GraphCreator.prototype.insertEdgeLabel = function (gEl, label, data) {
      var words = label.split(/\s+/g);
      ...
      link_label.style('text-anchor', 'middle')
        ...
        .attr('class', 'shadow').text(words);

      link_label.attr('x', function (d) {

        var node = d3.select(link_label.node().parentNode).selectAll('path').node();
    ...
    GraphCreator.prototype.updateGraph = function () {
      ...
      // add new paths
      const newPaths = paths.enter().append('path').style('marker-end', 'url(#end-arrow)').classed('link', true).attr('d', function (d) {
        return 'M' + d.source.x + ',' + d.source.y + 'L' + d.target.x + ',' + d.target.y;
      ...
      newPaths.each(function (d) {
        thisGraph.insertEdgeLabel(d3.select(this), 'Hello', d);
      });
  ...
  render() {
    this.generateMeme(window.d3, svg);
    return <div/>;
  }
}